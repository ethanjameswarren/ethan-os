...
| Cross-Domain Reasoning | In Development | Typed relationships (`supports`, `contradicts`, `applies_to`, `part_of`, etc.) and `links` on most objects; retrieval now follows direct and one-hop relationships in `scripts/core/universal_retrieval.py`. | Extend reasoning beyond one hop and add `skills/core/cross-domain-reasoning.md`. |
