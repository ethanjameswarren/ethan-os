...
2. Capture accounts and debts, grouped by institution. For each institution:
   a. Use `skills/finance/capture-account.md` for each account at that institution. Do not store account numbers, routing numbers, or credentials.
   b. For any liability accounts (credit cards, loans, mortgages) at that institution, immediately capture the corresponding debt using `skills/finance/capture-debt.md` — including interest rate, minimum payment, and any promotional terms. This avoids requiring the user to revisit the same institution later.
   c. Continue until the user indicates they are done with that institution, then move to the next.
...
6. Capture financial goals. Use `skills/finance/capture-financial-goal.md` for targets such as emergency fund, debt payoff, or savings.
7. If the user wants payoff or allocation guidance, run `skills/finance/plan-debt-payoff.md` and/or `skills/finance/allocate-next-dollar.md`. Present options; do not auto-apply a strategy.
8. Capture an initial financial snapshot. Use `skills/finance/capture-financial-snapshot.md` to record the starting position.
